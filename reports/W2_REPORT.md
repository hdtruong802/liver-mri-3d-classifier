...
> **Quy ước thuật ngữ.** Báo cáo giữ nguyên các thuật ngữ tiếng Anh đã quen dùng trong ngành và trong chính repo này (baseline, train/val/test, metric, leakage, cache, split, fold, epoch, model, run, pipeline, config, overfitting, calibration, registration...) vì dịch ra chỉ làm khó đọc hơn. Hai luật bắt buộc: (1) **không ghép trật tự từ tiếng Anh vào câu tiếng Việt** — viết "baseline chính thức", không viết "baseline official"; (2) **một khái niệm chỉ có một tên** xuyên suốt báo cáo, không lúc "gate" lúc "cổng kiểm".
...
**Về hạ tầng:** pipeline từ MRI thô đến bảng metric đã chạy được và tái lập được. Mọi gate an toàn khoa học đều đứng: split khoá ở mức bệnh nhân và đã commit, test chống leakage xanh, thống kê chuẩn hoá không xuyên bệnh nhân, và **test-104 chưa bị chạm một lần nào** — không có đường code nào dẫn tới nó.
