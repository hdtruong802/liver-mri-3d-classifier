...
| **Chạy CV trên Kaggle** | mở `notebooks/09_cv_runner.ipynb`, đặt `CONFIG_NAME` + `FOLDS` | sẵn sàng (W4). Đây là **runner chung** cho mọi config họ DenseNet: cổng 0 diff config so với baseline · dựng cache E4 nếu chưa có (~26 phút) · cổng kiểm cache · cổng ngân sách · TTA ngay sau train. Notebook 07 (bản khoá cứng vào baseline, logic dò đường dẫn đã sai) **đã xoá ở S-167** |
