...
| **⚠️ Build LẠI cache E4** (một lần, ~26 phút, **CPU**) | `python -m src.preprocess.build_cache --config configs/preprocess_e4.yaml` | Notebook dựng cache E4 **đã xoá ở S-168** cùng các notebook train phương pháp cũ. Cache E4 vẫn được **bốn** notebook còn lại MOUNT (`08`, `11_tta_e4`, `11_model_heatmaps`, `12_test104`) và bởi web app, nên nếu Kaggle Dataset chứa nó bị mất thì phải dựng lại bằng lệnh này. Cách nhanh nhất trên Kaggle: sao `notebooks/18_build_cache_cghnet.ipynb` (wrapper mỏng, **Accelerator = None**) và đổi config sang `preprocess_e4.yaml` |
